...
Public Sub VerifyStateChecksum()
    ...
    If Left$(rawContent, 5) = "ENC1:" Then
        ...
        ' Decrypt using the same logic as production (CollectHardware-based key)
        ...
        decrypted = modLicenseAudit.DecryptStateContent(encrypted)
        ...
        If Len(decrypted) > 0 Then
            ...
            End If
        Else
            Debug.Print "✗ ERROR: Could not decrypt data (DecryptData returned empty)."
            Debug.Print "  Possible reasons: hardware fingerprint mismatch or state corrupted."
        End If
    Else
